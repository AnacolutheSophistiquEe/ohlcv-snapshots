...
**Generated** : 2026-09-15T21:48:37.897414+00:00  
...
> ↳ spot €133.40 (+1.2% vs entrée) · entrée €131.83 · stop €121.28 · T1 €133.66 · R/R 0.17  
> ↳ P(T1 av. stop) 28 % _(réel 5 s)_ · EV/risk -0.039 _(réel 5 s)_ (GBM -0.078) · ¼-Kelly 0.059 · _first-passage 5 s RÉEL intra-séance (vrai ordre intrabar, n=80 séances) · non recalibrée track-record (n=0)_  
> ↳ stop −8.0% cohérent avec le bruit 5 s (EV-optimal ≈ −8.0%)  

> ⚠ **QA flags (1, dont 0 high)** — champs SUSPECTS (la section data fraîche prime) :
>   - **[MEDIUM]** §04 Pitchfork — Position dans le canal -80 % hors [0,100] (R² max 0.91). Canal dégénéré (bornes possiblement sous le prix) — à ne pas interpréter.
...
- Entry (zone de repli) : €131.47–€132.20 (mid €131.83)
- Spot actuel : €133.40 (+1.2% au-dessus de la zone — repli à attendre)
- Stop : €121.28 (stop swing_plan-based (-6.02%))
- Targets : T1 €133.66 · R/R 0.17 | T2 €135.48 · R/R 0.35 | T3 €137.30 · R/R 0.52
- Activation : entree LMT en attente de touche de zone
- Invalidation : close sous €121.28


## Risque mesuré — ce qui borne (et ce qui ne borne pas) la perte

- 🟠 **Régime de gap : intermediaire** — p_breach(-3 %)=1.64 % — entre les deux regimes ; ni queue pure ni franchissement ordinaire
- **Au stop du plan (6.02 %)** : le gap seul le franchit 0.469 % des séances (6 fois sur 1279).
   - exécution **2.148 pt plus bas** dans le cas TYPIQUE (médiane), 2.954 au p90, **3.576 au pire**
   - perte réelle **8.124 %** en moyenne _(tirée par la queue)_, jusqu'à **9.596 %** — au lieu des 6.02 % annoncés par la distance
...
- ⚠ **5 s — échantillon insuffisant sur : intraday (35.3 pt), swing (42.6 pt), deep (40.2 pt).** Ces chiffres peuvent être CITÉS, jamais servir à dimensionner ni à arbitrer entre deux plans.
...
- **β de baisse : 1.0101** (β de hausse 1.0948, asymétrie 0.9227) vs FCHI — 619 séances de repli, historique complet


## Echelle Warden — OU poser le stop

- **Verdict : AUCUN couple (stop, cible) ne tient les contraintes. Ce n'est pas un defaut du calcul : la structure est trop loin sous le spot pour qu'un stop structurel soit rentable a ces cibles. Le levier de fond n'est PAS la distance du stop mais la TAILLE de la ligne — voir `min_target_for_rr` pour savoir a partir de quelle cible chaque stop redeviendrait defendable. **MAIS ON POSE QUAND MEME** : `best_effort` porte le moins mauvais couple, non conforme et marque comme tel. Laisser la ligne NUE est pire — la perte y est non bornee.**
- **Couple retenu** : stop 119.6643 sur atr_grid (3.0 ATR, 10.297 %) — p(stop avant cible) 0.1157 [0.09 ; 0.15], R/R 2.19, perte reelle 10.297 % (gap inclus), CVaR 10.297 %, EV 0.5759 % — **NON CONFORME (best_effort — proposition de derniere main)**
   - viole : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 2.19 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
   - viole : R/R 2.19 < plancher 3.00 (mesure vs SPOT, gap inclus)
- Budget de queue : **12.0 %** du notionnel — ⚠ VALEUR FIGEE (valeur de repli (ligne absente de l'allocation)), PAS une mesure. L'allocation derivee de la contrainte du compte n'etait pas disponible.
- Candidats (la structure propose, la statistique elimine) :
   - ⚪ atr_based a 1.5 ATR (stop 5.148 %) — p(stop avant cible) 0.4191 [0.37 ; 0.47], R/R 2.776, perte reelle 8.124 % (gap inclus), EV -0.8979 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 2.78 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.78 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-0.90 %) : P(cible) 0.7 % x 22.55 % + P(rien) 57.4 % x 4.10 % ne couvrent pas P(stop) 41.9 % x 8.12 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ sr_based a 1.47 ATR (stop 7.362 %) — p(stop avant cible) 0.2551 [0.21 ; 0.30], R/R 2.706, perte reelle 8.333 % (gap inclus), EV 0.25 % — **REFUSE**
   ...
   - 🟢 support a 5.61 ATR (stop 21.557 %) — p(stop avant cible) 0.0021 [0.00 ; 0.01], R/R 1.046, perte reelle 21.557 % (gap inclus), EV 0.6684 % — **REFUSE**
   ...
   - ⚪ atr_grid a 0.25 ATR (stop 0.858 %) — p(stop avant cible) 0.9077 [0.87 ; 0.93], R/R 11.887, perte reelle 1.897 % (gap inclus), EV -0.9829 % — **REFUSE**
      - refuse : cible atteinte seulement 0.4 % du temps (< 15 %) meme a 10 seances : le R/R de 11.89 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      ...
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-0.98 %) : P(cible) 0.4 % x 22.55 % + P(rien) 8.9 % x 7.43 % ne couvrent pas P(stop) 90.8 % x 1.90 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ atr_grid a 0.5 ATR (stop 1.716 %) — p(stop avant cible) 0.7921 [0.75 ; 0.83], R/R 6.835, perte reelle 3.299 % (gap inclus), EV -1.2462 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 6.84 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      ...
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-1.25 %) : P(cible) 0.7 % x 22.55 % + P(rien) 20.1 % x 6.03 % ne couvrent pas P(stop) 79.2 % x 3.30 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ atr_grid a 0.75 ATR (stop 2.574 %) — p(stop avant cible) 0.687 [0.64 ; 0.73], R/R 5.088, perte reelle 4.432 % (gap inclus), EV -1.1982 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 5.09 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      ...
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-1.20 %) : P(cible) 0.7 % x 22.55 % + P(rien) 30.6 % x 5.52 % ne couvrent pas P(stop) 68.7 % x 4.43 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ atr_grid a 1.0 ATR (stop 3.432 %) — p(stop avant cible) 0.5891 [0.54 ; 0.64], R/R 4.124, perte reelle 5.468 % (gap inclus), EV -1.0225 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 4.12 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      ...
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-1.02 %) : P(cible) 0.7 % x 22.55 % + P(rien) 40.4 % x 5.06 % ne couvrent pas P(stop) 58.9 % x 5.47 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ grid_snapped a 1.47 ATR (stop 6.078 %) — p(stop avant cible) 0.3709 [0.32 ; 0.42], R/R 2.776, perte reelle 8.124 % (gap inclus), EV -0.5038 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 2.78 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.78 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-0.50 %) : P(cible) 0.7 % x 22.55 % + P(rien) 62.2 % x 3.78 % ne couvrent pas P(stop) 37.1 % x 8.12 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ atr_grid a 2.0 ATR (stop 6.864 %) — p(stop avant cible) 0.2999 [0.25 ; 0.35], R/R 2.776, perte reelle 8.124 % (gap inclus), EV -0.019 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 2.78 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.78 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-0.02 %) : P(cible) 0.7 % x 22.55 % + P(rien) 69.3 % x 3.26 % ne couvrent pas P(stop) 30.0 % x 8.12 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ atr_grid a 2.5 ATR (stop 8.581 %) — p(stop avant cible) 0.1898 [0.15 ; 0.23], R/R 2.35, perte reelle 9.596 % (gap inclus), EV 0.322 % — **REFUSE**
   ...
   - ⚪ atr_grid a 2.75 ATR (stop 9.439 %) — p(stop avant cible) 0.1493 [0.11 ; 0.19], R/R 2.35, perte reelle 9.596 % (gap inclus), EV 0.5462 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 2.35 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.35 < plancher 3.00 (mesure vs SPOT, gap inclus)
   - ⚪ atr_grid a 3.0 ATR (stop 10.297 %) — p(stop avant cible) 0.1157 [0.09 ; 0.15], R/R 2.19, perte reelle 10.297 % (gap inclus), EV 0.5759 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 2.19 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.19 < plancher 3.00 (mesure vs SPOT, gap inclus)
   - ⚪ atr_grid a 3.5 ATR (stop 12.013 %) — p(stop avant cible) 0.0771 [0.05 ; 0.11], R/R 1.877, perte reelle 12.013 % (gap inclus), EV 0.5719 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 1.88 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 1.88 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - refuse : CVaR 95 % 12.01 % > budget 12.00 %
   - ⚪ atr_grid a 4.0 ATR (stop 13.729 %) — p(stop avant cible) 0.0382 [0.02 ; 0.06], R/R 1.642, perte reelle 13.729 % (gap inclus), EV 0.5773 % — **REFUSE**
      - refuse : cible atteinte seulement 0.7 % du temps (< 15 %) meme a 10 seances : le R/R de 1.64 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 1.64 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - refuse : CVaR 95 % 13.73 % > budget 12.00 %
   - ⚪ atr_grid a 4.5 ATR (stop 15.445 %) — p(stop avant cible) 0.0187 [0.01 ; 0.04], R/R 1.46, perte reelle 15.445 % (gap inclus), EV 0.6233 % — **REFUSE**
   ...
   - ⚪ atr_grid a 5.0 ATR (stop 17.161 %) — p(stop avant cible) 0.0099 [0.00 ; 0.02], R/R 1.314, perte reelle 17.161 % (gap inclus), EV 0.6248 % — **REFUSE**
   ...
   - 🟢 grid_snapped a 5.61 ATR (stop 20.273 %) — p(stop avant cible) 0.0028 [0.00 ; 0.01], R/R 1.112, perte reelle 20.273 % (gap inclus), EV 0.6652 % — **REFUSE**
   ...
   - ⚪ atr_grid a 6.5 ATR (stop 22.309 %) — p(stop avant cible) 0.0021 [0.00 ; 0.01], R/R 1.011, perte reelle 22.309 % (gap inclus), EV 0.6668 % — **REFUSE**
   ...
   - ⚪ atr_grid a 7.0 ATR (stop 24.025 %) — p(stop avant cible) 0.0007 [0.00 ; 0.01], R/R 0.939, perte reelle 24.025 % (gap inclus), EV 0.6728 % — **REFUSE**
   ...
   - ⚪ atr_grid a 7.5 ATR (stop 25.742 %) — p(stop avant cible) 0.0007 [0.00 ; 0.01], R/R 0.876, perte reelle 25.742 % (gap inclus), EV 0.6716 % — **REFUSE**
   ...
   - ⚪ atr_grid a 8.0 ATR (stop 27.458 %) — p(stop avant cible) 0.0 [0.00 ; 0.01], R/R 0.821, perte reelle 27.458 % (gap inclus), EV 0.6767 % — **REFUSE**
...
- Reference : spot 133.4, ATR14 4.5786 (3.432 % du cours). **Les prix ci-dessous sont ancres sur ce spot de cloture : les REANCRER sur le prix vivant avant de poser.**
- **Horizon = consigne operateur.** intraday -> 1 seance ; SANS PRECISION -> swing, 5 seances ; positionnel/long -> 20.
- **Bornes appliquees d'office par le calculateur** : aucune tranche touchee plus de **45.0 %** du temps a l'horizon retenu ; aucune sous le **bruit journalier** du titre (0.349 ATR = 1.198 % du cours, soit l'excursion adverse mediane d'une seance) ; deux tranches jamais separees par moins que ce bruit. Ce sont des DEFAUTS DU MODULE : ils cedent avant toute consigne, et le rapport le dit.

| distance | % du cours | prix (spot ref) | p(touche) 1s | p(touche) 2s | p(touche) 3s | p(touche) 5s | p(touche) 10s | p(touche) 20s |
|---|---|---|---|---|---|---|---|---|
| 0.05 ATR | 0.172 % | 133.1711 | 87.83 % | 91.45 % | 93.22 % | 95.27 % | 96.93 % | 97.8 % |
| 0.1 ATR | 0.343 % | 132.9421 | 82.14 % | 87.92 % | 90.46 % | 93.3 % | 95.54 % | 96.9 % |
| 0.15 ATR | 0.515 % | 132.7132 | 75.27 % | 83.6 % | 87.02 % | 90.34 % | 94.06 % | 95.6 % |
| 0.2 ATR | 0.686 % | 132.4843 | 68.69 % | 78.49 % | 83.28 % | 88.37 % | 92.67 % | 94.9 % |
| 0.25 ATR | 0.858 % | 132.2554 | 62.12 % | 73.77 % | 79.25 % | 85.22 % | 90.59 % | 93.7 % |
| 0.35 ATR | 1.201 % | 131.7975 | 49.85 % | 64.54 % | 71.98 % | 79.11 % | 86.63 % | 91.5 % |
| 0.5 ATR | 1.716 % | 131.1107 | 34.94 % | 52.75 % | 61.55 % | 70.84 % | 80.3 % | 87.3 % |
| 0.75 ATR | 2.574 % | 129.9661 | 20.61 % | 36.74 % | 47.59 % | 59.01 % | 70.2 % | 80.7 % |
| 1.0 ATR | 3.432 % | 128.8214 | 10.7 % | 24.36 % | 34.81 % | 48.67 % | 61.49 % | 74.0 % |
| 1.25 ATR | 4.29 % | 127.6768 | 4.91 % | 16.21 % | 24.98 % | 39.61 % | 54.36 % | 67.6 % |
| 1.5 ATR | 5.148 % | 126.5321 | 2.45 % | 11.0 % | 18.39 % | 30.54 % | 46.63 % | 59.9 % |
| 2.0 ATR | 6.864 % | 124.2429 | 0.79 % | 5.11 % | 9.93 % | 19.21 % | 35.25 % | 50.5 % |
| 2.5 ATR | 8.581 % | 121.9536 | 0.49 % | 2.65 % | 5.6 % | 11.33 % | 24.65 % | 38.7 % |
| 3.0 ATR | 10.297 % | 119.6643 | 0.2 % | 1.67 % | 3.24 % | 7.09 % | 17.62 % | 30.2 % |
| 4.0 ATR | 13.729 % | 115.0857 | 0.1 % | 0.49 % | 0.88 % | 1.87 % | 7.52 % | 17.9 % |
| 6.0 ATR | 20.593 % | 105.9286 | 0.0 % | 0.0 % | 0.0 % | 0.2 % | 1.39 % | 4.4 % |
...
- **2 seance(s)** : plage utile 0.621–0.75 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 0.75 ATR (2.574 %, prix 129.9663), p(touche) 36.74 % (en stress 88.24 %)  ⚠ **SOLUTION DE COIN** — l'optimum est sur une borne, l'objectif est monotone : ce n'est PAS un arbitrage. Trancher avec la lecture inverse ci-dessus.  ⚠ **OPTIMUM NON IDENTIFIE** — au bootstrap par blocs, le vainqueur ne gagne que 47.6 % des re-echantillons : le rendement ne distingue pas les distances de cette zone. Trancher par la tolerance de sortie ou par un niveau structurel ne coute donc rien.
- **3 seance(s)** : plage utile 0.801–1.25 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 1.0 ATR (3.432 %, prix 128.8217), p(touche) 34.81 % (en stress 84.31 %)  ⚠ **OPTIMUM NON IDENTIFIE** — au bootstrap par blocs, le vainqueur ne gagne que 41.4 % des re-echantillons : le rendement ne distingue pas les distances de cette zone. Trancher par la tolerance de sortie ou par un niveau structurel ne coute donc rien.
- **5 seance(s)** : plage utile 1.101–1.5 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 1.25 ATR (4.29 %, prix 127.6771), p(touche) 39.61 % (en stress 93.14 %)  ✅ optimum identifie (62.9 % des re-echantillons)
- **10 seance(s)** : plage utile 1.572–2.5 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 2.0 ATR (6.864 %, prix 124.2434), p(touche) 35.25 % (en stress 99.01 %)  ✅ optimum identifie (70.9 % des re-echantillons)
- **20 seance(s)** : plage utile 2.233–3.0 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 2.5 ATR (8.581 %, prix 121.9529), p(touche) 38.7 % (en stress 98.0 %)  ✅ optimum identifie (69.4 % des re-echantillons)
...
- Régime probabiliste (posterior HMM, intraday) : bull 12.5 | bear 28.1 | side 59.4  _(probas d'ÉTAT de régime, bornées [5,85]% ; ≠ Monte-Carlo de l'EV ci-dessus)_
- Sizing : notional réel 267.0 (= 2 part(s) × prix) · cible 288.0


## Microstructure intraday (5 s réel · 80 séances)

- **First-passage & EV RÉELS par horizon** _(vérité terrain 5 s, **pondérés par récence** demi-vie ≈15.0 séances → régime des ~2-3 dernières semaines dominant ; entrée au DIP ; n_eff = échantillon effectif ; à comparer à l'EV GBM — le GBM tend à sur-estimer)_ :
  - **intraday** (entrée dip −1.174% → cible +1.384% / stop −8.0%, p_fill 55%, n_eff≈22.8) : P(cible|rempli) **28%** · **EV/risk -0.039** (×p_fill ; si rempli -0.57% du capital)
  - **swing** (entrée dip −2.588% → cible +3.095% / stop −3.523%, p_fill 44%, n_eff≈18.7) : P(cible|rempli) **50%** · **EV/risk +0.026** (×p_fill ; si rempli +0.21% du capital)
  - **deep** (entrée dip −4.002% → cible +4.377% / stop −5.363%, p_fill 35%, n_eff≈19.8) : P(cible|rempli) **63%** · **EV/risk +0.107** (×p_fill ; si rempli +1.64% du capital)
...
- **MACD** : hist -0.726  _(pas de croisement recent)_
- **BB** : %B 0.03 · largeur 8.4%
...
- **MA** : MA20 138.84 · MA50 136.46 · MA200 134.59  _(prix < MA20)_
- **Dist MA** : MA20 -3.9% · MA50 -2.2% · MA200 -0.9%


---

_Bulletin compact généré depuis `<TICKER>_report_data.json` (760886 bytes source)._  
