# MSTR

**Generated** : 2026-06-05T21:45:55.596757+00:00  
**Score** : 2/10 — **Rating** : Strong Pass (negative EV)  
**Subtitle** : strong_trend · volatilite low · $120.44  

## Régime & alignement multi-TF

- **Daily** : down (trend-down)  
- **H4** : down | **H1** : down  
- **Flag multi-TF** : triple_bearish (score 3)


## Lecture chartiste

Plan privilegie A (intraday), composite 2/10, conviction 'Strong Pass (negative EV)'.


## Niveaux clés & plan principal

**Plan A — intraday** (order_type LMT)
- Entry : $119.84–$121.04 (mid $120.44)
- Stop : $105.11 (stop atr-based (-12.72%))
- Targets : T1 $153.70 · R/R 2.17 | T2 $192.21 · R/R 4.68 | T3 $292.36 · R/R 11.21
- Activation : entree LMT en attente de touche de zone
- Invalidation : close sous $105.11


## Edge, scénarios & sizing

- EV/risk : -0.544 | EV/share : $-5.159 | p_fill : —
- Scenarios Monte-Carlo intraday : bull 40.8 | bear 54.2 | side 5.0
- Sizing : notional 0.0 | shares 0


## Positioning & factor

**Factor** : R² 0.63 · part idiosyncratique 0.18
**Short/Insider** : SI —% | insider — | verdict sell_bias_strong
**Options** : bearish


## Indicateurs (résumé)

- **RSI** : 14.4  _(survente)_
- **ADX** : 28.7  _(tendance etablie)_
- **MACD** : hist -5.725  _(pas de croisement recent)_
- **BB** : %B 0.02 · largeur 52.6%
- **ATR** : 10.22 (23.0e pct 1a)  _(volatilite normale)_
- **OBV/CMF** : OBV falling · CMF -0.012  _(neutre)_
- **Vol ratio** : 2.35  _(volume au-dessus de la moyenne)_
- **Choppiness** : 36.3  _(marche directionnel)_
- **MA** : MA20 160.96 · MA50 155.28 · MA200 201.86  _(prix < MA20)_
- **Dist MA** : MA20 -25.2% · MA50 -22.4% · MA200 -40.3%


---

_Bulletin compact généré depuis `<TICKER>_report_data.json` (13843 bytes source)._  
_Sans overlay Claude — fallback narratif pipeline baseline (à reviser pour enrichissement)._
