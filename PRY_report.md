...
**Generated** : 2026-09-24T21:56:17.845516+00:00  
**Santé technique** : 6/10 — **Rating** : Pass (negative EV)  
_(score = santé technique durable ; le rating = tradabilité/EV. Timing d'entrée distinct ci-dessous — audit §A3.)_  
**Subtitle** : indeterminate · volatilite normal · €122.35  

> ❄️ **EVENT-FROZEN** — horizon gelé jusqu'au 2026-09-30 — US PCE Price Index (headline) — Personal Income & Outlays (J-4 sess · macro taux)  
> ↳ spot €122.35 (+0.7% vs entrée) · entrée €121.53 · stop €116.83 · T1 €125.66 · R/R 0.88  
...
- Entry (zone de repli) : €120.71–€122.35 (mid €121.53)
- Spot actuel : €122.35 (+0.7% au-dessus de la zone — repli à attendre)
- Stop : €116.83 (stop swing_plan-based (-4.51%))
- Targets : T1 €125.66 · R/R 0.88 | T2 €129.79 · R/R 1.76 | T3 €133.92 · R/R 2.64
- Activation : entree LMT en attente de touche de zone
- Invalidation : close sous €116.83
...
- ⚠ **5 s — échantillon insuffisant sur : intraday (31.6 pt), swing (30.9 pt), deep (30.5 pt).** Ces chiffres peuvent être CITÉS, jamais servir à dimensionner ni à arbitrer entre deux plans.
...
- **β de baisse : 1.0378** (β de hausse 1.2292, asymétrie 0.8443) vs FTSEMIB — 565 séances de repli, historique complet
   - ⚠ le β de baisse récent vaut 1.495× celui de l'historique complet : la sensibilité du titre au marché a changé.


## Echelle Warden — OU poser le stop

- **Verdict : AUCUN couple (stop, cible) ne tient les contraintes. Ce n'est pas un defaut du calcul : la structure est trop loin sous le spot pour qu'un stop structurel soit rentable a ces cibles. Le levier de fond n'est PAS la distance du stop mais la TAILLE de la ligne — voir `min_target_for_rr` pour savoir a partir de quelle cible chaque stop redeviendrait defendable. **MAIS ON POSE QUAND MEME** : `best_effort` porte le moins mauvais couple, non conforme et marque comme tel. Laisser la ligne NUE est pire — la perte y est non bornee.**
- **Couple retenu** : stop 108.2393 sur atr_grid (3.0 ATR, 11.533 %) — p(stop avant cible) 0.0991 [0.07 ; 0.13], R/R 2.289, perte reelle 11.533 % (gap inclus), CVaR 11.533 %, EV 1.2813 % — **NON CONFORME (best_effort — proposition de derniere main)**
...
- Candidats (la structure propose, la statistique elimine) :
   - ⚪ sr_based a 0.41 ATR (stop 4.027 %) — p(stop avant cible) 0.5864 [0.53 ; 0.64], R/R 4.714, perte reelle 5.601 % (gap inclus), EV -0.7084 % — **REFUSE**
      - refuse : cible atteinte seulement 1.2 % du temps (< 15 %) meme a 10 seances : le R/R de 4.71 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      ...
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-0.71 %) : P(cible) 1.2 % x 26.40 % + P(rien) 40.1 % x 5.62 % ne couvrent pas P(stop) 58.6 % x 5.60 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ atr_based a 1.5 ATR (stop 5.767 %) — p(stop avant cible) 0.4264 [0.38 ; 0.48], R/R 3.338, perte reelle 7.909 % (gap inclus), EV -0.4067 % — **REFUSE**
      ...
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-0.41 %) : P(cible) 1.2 % x 26.40 % + P(rien) 56.1 % x 4.71 % ne couvrent pas P(stop) 42.6 % x 7.91 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ swing_based a 1.34 ATR (stop 7.583 %) — p(stop avant cible) 0.277 [0.23 ; 0.33], R/R 2.641, perte reelle 9.998 % (gap inclus), EV 0.2829 % — **REFUSE**
      - refuse : cible atteinte seulement 1.6 % du temps (< 15 %) meme a 10 seances : le R/R de 2.64 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.64 < plancher 3.00 (mesure vs SPOT, gap inclus)
   - 🔴 support a 3.91 ATR (stop 17.46 %) — p(stop avant cible) 0.0129 [0.00 ; 0.03], R/R 1.512, perte reelle 17.46 % (gap inclus), EV 1.5035 % — **REFUSE**
      ...
      - ⚠ support DETECTE a 0.31 ATR du spot — compartiment <1, mesure a 47.1 % de casse (IC clusterise [0.441 ; 0.501] sur 1230 touches, registre point-in-time). C'est un pile ou face : l'ancrage n'apporte rien de plus qu'une distance arbitraire et rapproche le stop du bruit. Si c'est le seul disponible, la ligne n'est pas ancrable et le levier redevient la TAILLE.
   - 🟢 support a 9.31 ATR (stop 38.22 %) — p(stop avant cible) 0.0 [0.00 ; 0.01], R/R 0.691, perte reelle 38.22 % (gap inclus), EV 1.502 % — **REFUSE**
   ...
   - ⚪ grid_snapped a 0.41 ATR (stop 2.739 %) — p(stop avant cible) 0.6993 [0.65 ; 0.75], R/R 6.278, perte reelle 4.205 % (gap inclus), EV -0.7273 % — **REFUSE**
      - refuse : cible atteinte seulement 1.2 % du temps (< 15 %) meme a 10 seances : le R/R de 6.28 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      ...
      - 🚩 **LIGNE_EV_NEGATIVE** — le meilleur bracket disponible perd de l'argent en esperance (-0.73 %) : P(cible) 1.2 % x 26.40 % + P(rien) 28.8 % x 6.56 % ne couvrent pas P(stop) 69.9 % x 4.21 %.
        -> l'alternative dominante n'est pas un autre stop mais la REDUCTION ou la CLOTURE de la ligne. A remonter a l'etage portefeuille, pas a traiter en assouplissant les contraintes.
   - ⚪ grid_snapped a 1.34 ATR (stop 6.295 %) — p(stop avant cible) 0.3897 [0.34 ; 0.44], R/R 3.338, perte reelle 7.909 % (gap inclus), EV 0.0404 % — **REFUSE**
   ...
   - ⚪ atr_grid a 2.25 ATR (stop 8.65 %) — p(stop avant cible) 0.2424 [0.20 ; 0.29], R/R 2.641, perte reelle 9.998 % (gap inclus), EV 0.5312 % — **REFUSE**
      - refuse : cible atteinte seulement 1.6 % du temps (< 15 %) meme a 10 seances : le R/R de 2.64 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.64 < plancher 3.00 (mesure vs SPOT, gap inclus)
   - ⚪ atr_grid a 2.5 ATR (stop 9.611 %) — p(stop avant cible) 0.194 [0.15 ; 0.24], R/R 2.641, perte reelle 9.998 % (gap inclus), EV 0.9217 % — **REFUSE**
      - refuse : cible atteinte seulement 1.6 % du temps (< 15 %) meme a 10 seances : le R/R de 2.64 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 2.64 < plancher 3.00 (mesure vs SPOT, gap inclus)
   - ⚪ atr_grid a 2.75 ATR (stop 10.572 %) — p(stop avant cible) 0.1477 [0.11 ; 0.19], R/R 2.497, perte reelle 10.572 % (gap inclus), EV 1.1491 % — **REFUSE**
   ...
   - ⚪ atr_grid a 3.0 ATR (stop 11.533 %) — p(stop avant cible) 0.0991 [0.07 ; 0.13], R/R 2.289, perte reelle 11.533 % (gap inclus), EV 1.2813 % — **REFUSE**
   ...
   - ⚪ atr_grid a 3.5 ATR (stop 13.455 %) — p(stop avant cible) 0.0632 [0.04 ; 0.09], R/R 1.962, perte reelle 13.455 % (gap inclus), EV 1.3173 % — **REFUSE**
      - refuse : cible atteinte seulement 1.6 % du temps (< 15 %) meme a 10 seances : le R/R de 1.96 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 1.96 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - refuse : CVaR 95 % 13.46 % > budget 12.00 %
   - 🔴 grid_snapped a 3.91 ATR (stop 16.172 %) — p(stop avant cible) 0.0184 [0.01 ; 0.04], R/R 1.633, perte reelle 16.172 % (gap inclus), EV 1.5109 % — **REFUSE**
   ...
   - ⚪ atr_grid a 5.0 ATR (stop 19.222 %) — p(stop avant cible) 0.0104 [0.00 ; 0.03], R/R 1.373, perte reelle 19.222 % (gap inclus), EV 1.4981 % — **REFUSE**
      - refuse : cible atteinte seulement 1.6 % du temps (< 15 %) meme a 10 seances : le R/R de 1.37 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 1.37 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - refuse : CVaR 95 % 19.22 % > budget 12.00 %
   - ⚪ atr_grid a 5.5 ATR (stop 21.144 %) — p(stop avant cible) 0.0086 [0.00 ; 0.02], R/R 1.249, perte reelle 21.144 % (gap inclus), EV 1.4849 % — **REFUSE**
   ...
   - ⚪ atr_grid a 6.0 ATR (stop 23.066 %) — p(stop avant cible) 0.0067 [0.00 ; 0.02], R/R 1.145, perte reelle 23.066 % (gap inclus), EV 1.4835 % — **REFUSE**
      - refuse : cible atteinte seulement 1.6 % du temps (< 15 %) meme a 10 seances : le R/R de 1.14 est un rapport de distances, pas une esperance — viser si loin revient a n'avoir pas de cible
      - refuse : R/R 1.14 < plancher 3.00 (mesure vs SPOT, gap inclus)
      - refuse : CVaR 95 % 23.07 % > budget 12.00 %
   - ⚪ atr_grid a 6.5 ATR (stop 24.988 %) — p(stop avant cible) 0.0041 [0.00 ; 0.02], R/R 1.057, perte reelle 24.988 % (gap inclus), EV 1.4952 % — **REFUSE**
   ...
   - ⚪ atr_grid a 7.0 ATR (stop 26.91 %) — p(stop avant cible) 0.0034 [0.00 ; 0.01], R/R 0.981, perte reelle 26.91 % (gap inclus), EV 1.4948 % — **REFUSE**
   ...
   - ⚪ atr_grid a 7.5 ATR (stop 28.833 %) — p(stop avant cible) 0.0014 [0.00 ; 0.01], R/R 0.916, perte reelle 28.833 % (gap inclus), EV 1.4996 % — **REFUSE**
   ...
   - ⚪ atr_grid a 8.0 ATR (stop 30.755 %) — p(stop avant cible) 0.0014 [0.00 ; 0.01], R/R 0.858, perte reelle 30.755 % (gap inclus), EV 1.4969 % — **REFUSE**
...
- Reference : spot 122.35, ATR14 4.7036 (3.844 % du cours). **Les prix ci-dessous sont ancres sur ce spot de cloture : les REANCRER sur le prix vivant avant de poser.**
- **Horizon = consigne operateur.** intraday -> 1 seance ; SANS PRECISION -> swing, 5 seances ; positionnel/long -> 20.
- **Bornes appliquees d'office par le calculateur** : aucune tranche touchee plus de **45.0 %** du temps a l'horizon retenu ; aucune sous le **bruit journalier** du titre (0.344 ATR = 1.322 % du cours, soit l'excursion adverse mediane d'une seance) ; deux tranches jamais separees par moins que ce bruit. Ce sont des DEFAUTS DU MODULE : ils cedent avant toute consigne, et le rapport le dit.

| distance | % du cours | prix (spot ref) | p(touche) 1s | p(touche) 2s | p(touche) 3s | p(touche) 5s | p(touche) 10s | p(touche) 20s |
|---|---|---|---|---|---|---|---|---|
| 0.05 ATR | 0.192 % | 122.1148 | 91.88 % | 93.95 % | 94.74 % | 95.53 % | 97.3 % | 97.88 % |
| 0.1 ATR | 0.384 % | 121.8796 | 85.25 % | 88.9 % | 91.17 % | 92.94 % | 95.2 % | 96.27 % |
| 0.15 ATR | 0.577 % | 121.6445 | 77.82 % | 84.34 % | 87.5 % | 90.46 % | 93.01 % | 94.25 % |
| 0.2 ATR | 0.769 % | 121.4093 | 69.6 % | 78.99 % | 82.54 % | 86.78 % | 90.71 % | 92.23 % |
| 0.25 ATR | 0.961 % | 121.1741 | 61.88 % | 74.13 % | 78.47 % | 83.1 % | 88.11 % | 90.31 % |
| 0.35 ATR | 1.346 % | 120.7037 | 49.21 % | 63.63 % | 70.73 % | 76.64 % | 83.82 % | 87.49 % |
| 0.5 ATR | 1.922 % | 119.9982 | 34.85 % | 51.64 % | 59.82 % | 67.69 % | 76.52 % | 81.84 % |
| 0.75 ATR | 2.883 % | 118.8223 | 19.01 % | 34.09 % | 42.86 % | 54.17 % | 64.74 % | 73.16 % |
| 1.0 ATR | 3.844 % | 117.6464 | 9.8 % | 22.99 % | 31.25 % | 43.84 % | 55.34 % | 64.88 % |
| 1.25 ATR | 4.805 % | 116.4705 | 5.54 % | 15.66 % | 23.61 % | 34.0 % | 46.95 % | 57.11 % |
| 1.5 ATR | 5.767 % | 115.2946 | 2.48 % | 9.42 % | 15.87 % | 23.96 % | 36.36 % | 48.54 % |
| 2.0 ATR | 7.689 % | 112.9429 | 0.4 % | 3.96 % | 7.54 % | 13.62 % | 24.18 % | 37.13 % |
| 2.5 ATR | 9.611 % | 110.5911 | 0.0 % | 1.59 % | 3.37 % | 7.95 % | 15.58 % | 26.54 % |
| 3.0 ATR | 11.533 % | 108.2393 | 0.0 % | 0.79 % | 1.69 % | 4.17 % | 9.69 % | 18.97 % |
| 4.0 ATR | 15.377 % | 103.5357 | 0.0 % | 0.1 % | 0.6 % | 1.79 % | 4.8 % | 11.1 % |
| 6.0 ATR | 23.066 % | 94.1286 | 0.0 % | 0.0 % | 0.0 % | 0.4 % | 1.8 % | 3.63 % |
...
- **2 seance(s)** : plage utile 0.595–0.75 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 0.75 ATR (2.883 %, prix 118.8226), p(touche) 34.09 % (en stress 86.14 %)  ⚠ **SOLUTION DE COIN** — l'optimum est sur une borne, l'objectif est monotone : ce n'est PAS un arbitrage. Trancher avec la lecture inverse ci-dessus.  ⚠ **OPTIMUM NON IDENTIFIE** — au bootstrap par blocs, le vainqueur ne gagne que 31.4 % des re-echantillons : le rendement ne distingue pas les distances de cette zone. Trancher par la tolerance de sortie ou par un niveau structurel ne coute donc rien.
- **3 seance(s)** : plage utile 0.718–1.25 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 0.75 ATR (2.883 %, prix 118.8226), p(touche) 42.86 % (en stress 94.06 %)  ⚠ **OPTIMUM NON IDENTIFIE** — au bootstrap par blocs, le vainqueur ne gagne que 56.1 % des re-echantillons : le rendement ne distingue pas les distances de cette zone. Trancher par la tolerance de sortie ou par un niveau structurel ne coute donc rien.
- **5 seance(s)** : plage utile 0.972–1.5 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 1.0 ATR (3.844 %, prix 117.6469), p(touche) 43.84 % (en stress 99.01 %)  ⚠ **OPTIMUM NON IDENTIFIE** — au bootstrap par blocs, le vainqueur ne gagne que 44.1 % des re-echantillons : le rendement ne distingue pas les distances de cette zone. Trancher par la tolerance de sortie ou par un niveau structurel ne coute donc rien.
- **10 seance(s)** : plage utile 1.296–2.0 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 1.5 ATR (5.767 %, prix 115.2941), p(touche) 36.36 % (en stress 98.02 %)  ⚠ **OPTIMUM NON IDENTIFIE** — au bootstrap par blocs, le vainqueur ne gagne que 42.2 % des re-echantillons : le rendement ne distingue pas les distances de cette zone. Trancher par la tolerance de sortie ou par un niveau structurel ne coute donc rien.
- **20 seance(s)** : plage utile 1.655–3.0 ATR _(borne basse : tolerance de sortie par defaut (45 %))_ — optimum 2.0 ATR (7.689 %, prix 112.9425), p(touche) 37.13 % (en stress 99.0 %)  ⚠ **OPTIMUM NON IDENTIFIE** — au bootstrap par blocs, le vainqueur ne gagne que 58.5 % des re-echantillons : le rendement ne distingue pas les distances de cette zone. Trancher par la tolerance de sortie ou par un niveau structurel ne coute donc rien.
...
- Régime probabiliste (posterior HMM, swing) : bull 85.2 | bear 6.9 | side 7.9  _(probas d'ÉTAT de régime, bornées [5,85]% ; ≠ Monte-Carlo de l'EV ci-dessus)_
...
- **First-passage & EV RÉELS par horizon** _(vérité terrain 5 s, **pondérés par récence** demi-vie ≈15.0 séances → régime des ~2-3 dernières semaines dominant ; entrée au DIP ; n_eff = échantillon effectif ; à comparer à l'EV GBM — le GBM tend à sur-estimer)_ :
  - **intraday** (entrée dip −0.304% → cible +1.52% / stop −8.0%, p_fill 84%, n_eff≈35.1) : P(cible|rempli) **42%** · **EV/risk -0.072** (×p_fill ; si rempli -0.69% du capital)
  - **swing** (entrée dip −0.665% → cible +3.398% / stop −3.87%, p_fill 88%, n_eff≈35.2) : P(cible|rempli) **30%** · **EV/risk -0.329** (×p_fill ; si rempli -1.45% du capital)
  - **deep** (entrée dip −1.024% → cible +4.806% / stop −5.826%, p_fill 93%, n_eff≈37.2) : P(cible|rempli) **28%** · **EV/risk -0.406** (×p_fill ; si rempli -2.56% du capital)
...
- **MACD** : hist 0.505  _(pas de croisement recent)_
...
- **Dist MA** : MA20 -0.5% · MA50 -1.1% · MA200 +3.8%


---

_Bulletin compact généré depuis `<TICKER>_report_data.json` (872758 bytes source)._  
